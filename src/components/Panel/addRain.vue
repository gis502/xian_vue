    ...
    async confirmRainPoint() {
      ...
      if (this.adminArea) {
        ...
        let entity = {
          ...
          id: "test_rain",
          trigger: "暴雨",
          rainfall: this.rainfallArry[0], // 使用匹配的索引获取降雨量
          duration: this.durationArry[0], // 使用匹配的索引获取持续时间
...
<!--<template>
  &lt;!&ndash; 新增v-show控制面板显示/隐藏，绑定showInfoPanel状态 &ndash;&gt;
  <div class="rain-info-panel" :style="styleObject">
    <div class="panel-title">暴雨信息</div>
    <div class="panel-content">
      <div
      ...
      <div class="button-group">
        <button
            @click="confirmRainPoint"
            :disabled="!entries.some(entry => entry.rainfall > 0 && entry.duration > 0)"
            style="width: 80px">
          确认添加
        </button>
        <button @click="cancelRainPoint" style="width: 80px">取消</button>
...
export default {
  ...
  props: {
    ...
    PanelPosition: {
      type: Object,
      required: true,
      default: () => ({x: 100, y: 100}) // 新增默认位置，避免定位异常
    },
    // 新增：控制面板显示/隐藏的状态（关键修复点）
    showInfoPanel: {
      type: Boolean,
      required: true,
      default: false
  ...
  watch: {
    ...
    // 新增：监听显示状态，确保面板在显示时正确初始化
    showInfoPanel(newVal) {
      if (newVal) {
        this.initPanelData(); // 显示时初始化数据
      }
    }
  },
  computed: {
    styleObject() {
      return {
        ...
        left: `${this.positionEntity.x}px`,
        top: `${this.positionEntity.y}px`,
        zIndex: 9999, // 新增：提高层级，避免被其他元素遮挡
        backgroundColor: "white", // 新增：确保背景可见
        padding: "10px",
  ...
  methods: {
    ...
    initPanelData() {
      ...
      // 补充：获取标记点所在区域（修复可能的区域信息缺失问题）
      if (this.selectedPositionLonAndLat) {
        this.adminArea = layers.getAdministrationByPoint(
            this.selectedPositionLonAndLat.longitude,
            this.selectedPositionLonAndLat.latitude
        )?.name || '西安市';
      }
    },
    ...
    async confirmRainPoint() {
      ...
      this.entries.forEach(entry => {
        if (entry.rainfall > 0) {
          this.positionArry.push(entry.name);
          this.rainfallArry.push(entry.rainfall);
          this.durationArry.push(entry.duration || 0);
        }
      });
      ...
      if (this.adminArea) {
        ...
        this.$emit('update:loading-model', false);
        // 关闭面板时通知父组件更新状态
        this.$emit('update:showInfoPanel', false);
      } else {
        console.log("未找到标记点所在的行政区划");
      }
    },
    ...
    async processAllDistricts() {
      ...
      console.log("所有有效区县汇总数据：", allMatchedHuapoData, allPointSet);
