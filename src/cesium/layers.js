...
let layers = {
  ...
  DrawEllipse(longitude, latitude, magnitude) {
    ...
    let min_line = this.pointToLineDistance_getMinLine(
      { longitude, latitude },
      lineData
    );
    ...
    //计算角度
    let bearing = this.calculateStrikeDirection(
      first_point[0],
      first_point[1],
      last_point[0],
      last_point[1]
  ...
  DrawCircle(point, bearing, magnitude) {
    ...
    // 设置公共变量
    this.constant.rotation = rotation;
    this.constant.longitude = position.x;
    this.constant.latitude = position.y;
    // 循环创建多个同心椭圆，长轴方向与断裂带走向一致
    ellipseParams.forEach((params) => {
      // 设置公共变量长短轴
      this.constant.majorAxis =
        params.semiMajorAxis > this.constant.majorAxis
          ? params.semiMajorAxis
          : this.constant.majorAxis;
      this.constant.minorAxis =
        params.semiMinorAxis > this.constant.minorAxis
      ...
      let ellipse = new Cesium.Entity({
        ...
        ellipse: {
          ...
          material: Cesium.Color.fromCssColorString(params.color).withAlpha(
            0.3
          ),
          height: 0,
          outline: true,
  ...
  /**
   * 判断点是否在椭圆内
   * @param {number[]} testPointLonLat - 待测点的经纬度 [经度, 纬度]
   * @returns {boolean} - 点是否在椭圆内
   */
  isPointInEllipse: function (testPointLonLat) {
    // 椭圆中心笛卡尔坐标
    const ellipseCenterCartesian = Cesium.Cartesian3.fromDegrees(
      this.constant.longitude,
      this.constant.latitude
    );
    const viewer = window.viewer;
    const ellipsoid = viewer.scene.globe.ellipsoid;

    // 1. 将待测点转换为笛卡尔坐标
    const testPointCartesian = Cesium.Cartesian3.fromDegrees(
      testPointLonLat[0],
      testPointLonLat[1]
    );
    ...
    const testPointENU = Cesium.Matrix4.multiplyByPoint(
      inverseMatrix,
      testPointCartesian,
      new Cesium.Cartesian3()
    );
    ...
    // 4. 反向旋转坐标（校正椭圆旋转）
    const rotation = -this.constant.rotation; // 反向旋转角度
    const cosAngle = Math.cos(rotation);
    const sinAngle = Math.sin(rotation);

    // 应用旋转矩阵 (绕Z轴)
    const xRotated = testPointENU.x * cosAngle - testPointENU.y * sinAngle;
    const yRotated = testPointENU.x * sinAngle + testPointENU.y * cosAngle;

    // 5. 验证椭圆方程 (x^2/a^2 + y^2/b^2 <= 1)
    const a = this.constant.majorAxis; // 长半轴
    const b = this.constant.minorAxis; // 短半轴
    const normalizedX = xRotated / a;
    const normalizedY = yRotated / b;
